...
export function buildHelpMessage() {
  ...
  return [
    ...
    "",
    "**Common flows**",
    "🚀 **New server** → `/setup run` → choose free structure or unlock",
    "💎 **Bought on Ko-fi** → `/setup redeem` then `/setup unlock`",
    "🎁 **Got a free grant** → `/setup unlock` _(after interview)_",
    "",
    "**`/setup` commands** _(server owner)_",
    "• `/setup run` — AI interview in DMs (saves blueprint)",
    "• `/setup redeem` — apply Ko-fi code to your account",
    "• `/setup unlock` — roles, embeds, pins & tickets",
    "• `/setup post-messages` — re-post rules/about/FAQ",
    "• `/setup ticket-panel` — refresh the ticket menu",
    "• `/setup edit-channel` — topic / pin helpers",
    "• `/setup edit-message` — edit a bot embed",
    "• `/setup nuke` — backup + wipe channels _(danger)_",
    "",
